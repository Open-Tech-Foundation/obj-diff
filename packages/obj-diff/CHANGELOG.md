...
### Added
- `serialize(diff)` / `deserialize(wire)` — a self-describing JSON wire format for diffs. `diff()` returns live objects by reference, so `JSON.stringify` mangles them (`Date`→string, `Map`/`Set`→`{}`, `TypedArray`→numeric-keyed object, `BigInt`→throws); `serialize` encodes every supported value (including inside `path` and nested values) with `_t`/`_v` type envelopes, and `deserialize` restores the exact types so a `patch()` on the far side reconstructs the correct objects. Covers `Date`, `RegExp`, `Map`, `Set`, all `TypedArray`s, `ArrayBuffer`, `DataView`, `Error` (+ standard subclasses), boxed primitives, `BigInt`, `NaN`/`±Infinity`/`-0`, `undefined`, and `Temporal` values. Symbols, functions, class instances, and circular references throw. Deserializing a `Temporal` value needs a `Temporal` implementation on `globalThis`.
