...
`Date`, `RegExp`, `Map`, `Set`, all `TypedArray`s, `ArrayBuffer`, `DataView`, `Error` (and standard subclasses), boxed primitives, `BigInt`, `NaN` / `±Infinity` / `-0`, `undefined`, and every `Temporal` type — **nested to any depth**, including `Map`/`Set` values and object keys that appear in a `path`.
