...
`Temporal` values are treated as atomic — a change replaces the whole value. `Duration` is compared structurally (by its canonical string), because two equal-length durations expressed differently (`PT2H` vs `PT120M`) are genuinely different values.
